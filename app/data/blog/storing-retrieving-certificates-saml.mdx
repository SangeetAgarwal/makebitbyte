...
I used ComponentSpace SAML library which provides quite a few options to store certificates such as local files system,
...
3. CI/CD,
   ...
   2. Most importantly, likely you'll have multiple environments and each your developemnt/test environment and production/staging might/should have its own
      certificate. This means each environment will have its own appsettings file and you'll need to customize your
...
Finally, I used the following code to retrieve the certificate from the key vault.
...
Component Spaces' SAML library accesses the private key string using IConfiguration[“Idp”] where `Idp` is the key specified in the appsettings file.
...
So, the following steps are needed to access the key vault from local development environment.
